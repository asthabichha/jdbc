...
public class Customers {
		 ...
		 try 
		  {
			//load the driver class  
			 Class.forName("oracle.jdbc.driver.OracleDriver");
				 con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","astha","aakash");
		  } 
		  catch (Exception e) 
		  {
	...
	public void getCustomer() 
	{
	  try {
	   ...
	   res = st.executeQuery("select * from customer order by cusId");
	   System.out.println("CusId\t CustomerName\t City \t\t Contact No");
	   System.out.println("-----------------------------------------------------------");
	  while (res.next()) 
	  {
	...
	public void deleteCustomer(int cid) 
	{
		try {
			con = getConnection();
		    ps = con.prepareStatement("delete from customer where cusId=?");
		    ps.setInt(1, cid);
		    int i = ps.executeUpdate();
		    if (i != 0) {
